...
echo "▶️ Mise à jour de SoundEngine avec une waveform SNARE…"
...
public class SoundEngine {
    ...
    public enum Waveform { SINE, SQUARE, TRIANGLE, CLICK, NOISE, SNARE }
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (wf) {
                ...
                case NOISE:
                    ...
                    break;
                case SNARE: {
                    // pseudo-caisse claire : bruit + petite composante tonale
                    double noise = (Math.random() * 2.0 - 1.0);
                    double tone  = Math.sin(twoPiF * i); // corps de la caisse
...
echo "▶️ Mise à jour de MainActivity pour utiliser SNARE sur les pulses…"
...
public class MainActivity extends AppCompatActivity {
    ...
    // mémorisation des derniers taps
    ...
    private final Runnable tickRunnable = new Runnable() {
        @Override public void run() {
            ...
            if (pattern != null && currentStep < pattern.length && pattern[currentStep]) {
                soundEngine.playPulse(); // SNARE
            } else {
                soundEngine.playStep();  // petit clic discret
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soundEngine = new SoundEngine(
                44_100,
                SoundEngine.Waveform.CLICK, 4000.0, 30, 2, 30,
...
echo "✅ SoundEngine + MainActivity mis à jour avec snare sur les pulses."
