...
echo "▶️ Mise à jour de SoundEngine (noteGain réglable)…"
 ...
 *
 * Le volume relatif des notes est contrôlé par setNoteGain(0..1).
 */
public class SoundEngine {
    ...
    // gain des notes (0 = notes muettes, 1 = notes pleinement présentes)
    ...
    public void release() {
        try { if (kickTrack      != null) kickTrack.release();      } catch (Throwable ignored) {}
        try { if (snareTrack     != null) snareTrack.release();     } catch (Throwable ignored) {}
        try { if (hatOpenTrack   != null) hatOpenTrack.release();   } catch (Throwable ignored) {}
        try { if (hatClosedTrack != null) hatClosedTrack.release(); } catch (Throwable ignored) {}
    }

    private void rebuildTracks() {
        // Libérer d'abord d'éventuels anciens tracks
        releaseInternal();
     ...
     * - composante "drum" (bruit + tonalité de base)
     * - composante "note" (sinusoïde LA/DO/MI/SOL)
     *   pondérée par noteGain.
     */
    private short[] synthBuffer(Waveform wf,
        ...
        // Fréquences des notes (une octave confortable)
        double noteFreqHz;
        switch (wf) {
...
echo "✅ SoundEngine mis à jour (support noteGain)."
